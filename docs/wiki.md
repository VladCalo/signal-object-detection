...
### v9 — HistGradientBoosting with Viridis-Decoded Preprocessed Features
**Date**: 30 May 2026  
**Kaggle score**: *(pending — submit `submissions/v9_hgbc_preprocessed_best.csv`)*  
**Submission file**: `submissions/v9_hgbc_preprocessed_best.csv`

#### Key Insight: Viridis Colormap Decoding

The competition images are **not natural RGB images** — they are single-channel intensity spectrograms rendered using matplotlib's **viridis colormap**. Every pixel maps exactly to one of the 256 viridis LUT entries (verified: 100% exact match across all images).

Previous experiments (v1–v8) treated R, G, B as 3 independent channels, which is **fundamentally wrong**: the three channels are deterministic functions of a single scalar. The Alpha channel (always 255) wasted 25% of features.

By reverse-mapping each pixel's RGB back to the original intensity (0–255) using the viridis LUT, we recover the **true underlying signal data**.

#### Feature Extraction — Preprocessed Decoded Features

Starting from the decoded single-channel intensity image (128×55):

| # | Feature Group | Count | Description |
|---|---|---|---|
| 1 | Raw decoded pixel intensities | 7,040 | Full spatial layout of the decoded signal |
| 2 | Background-subtracted pixels | 7,040 | Per-row median subtraction removes frequency-dependent background noise |
| 3 | Signal column profile | 55 | Mean of background-subtracted image per column (normalized) |
| 4 | Thresholded signal profiles | 228 | Binary column profiles at thresholds 10/20/30/50 + connected components |
| 5 | Adaptive threshold profile | 56 | Per-column threshold at mean + 1.5×std |
| 6 | Local contrast normalized profiles | 110 | Column mean & std after local contrast normalization (kernel=5) |
| | **Total** | **~14,529** | |

The background subtraction (group 2) is the most impactful addition: since signals are narrow vertical traces, the per-row median captures the frequency-dependent background noise floor. Subtracting it isolates the signal traces.

#### Model: HistGradientBoostingClassifier
- `sklearn.ensemble.HistGradientBoostingClassifier` — histogram-based gradient boosting (similar to LightGBM)
- Handles high-dimensional features natively via histogram binning — no PCA or scaling needed
- Hyperparameter search: **max_iter ∈ {1000, 2000, 3000}**, **max_depth ∈ {6, 8}**, **learning_rate ∈ {0.02, 0.05}**
- `min_samples_leaf=20` fixed (regularization)
- Train/val split: **80/20** (12,400 train, 3,100 val)

#### Validation Results

*(to be filled after running `python solution_hgbc.py`)*
